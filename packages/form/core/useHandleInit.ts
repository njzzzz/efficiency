...
export function initFormItem(
  ...
  if (item.list) {
    ...
    initFormList(item.list, runtimeModel, elFormRef, runtimeSchema, topUtils);
    return;
  }
  ...
  // 此处调用顺序不能发生错误
  rewriteItemListeners(item, runtimeModel, runtimeSchema, topUtils);
  patchReactiveProps(item, runtimeSchema);
  generatorDependOn(
    item,
    runtimeModel,
  ...
  generatorRules(item, runtimeModel, runtimeSchema, elFormRef, topUtils);
...
export function initFormList(
  ...
  for (let index = 0; index < list.length; index++) {
    ...
    initFormItem(item, runtimeModel, elFormRef, runtimeSchema, topUtils);
  }
}

export function useHandleInit() {
  function init(list, runtimeModel, elFormRef, runtimeSchema, topUtils) {
    initFormList(list, runtimeModel, elFormRef, runtimeSchema, topUtils);
  }
  return {
    init,
  };
